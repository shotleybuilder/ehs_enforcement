defmodule EhsEnforcement.Integrations.Airtable.Post do
  ...
  defp make_airtable_dataset(records) when is_list(records) do
    formatted_records = Enum.map(records, fn record ->
      case record do
        %{"fields" => _} -> record  # Already in Airtable format
        fields -> %{"fields" => fields}  # Wrap in fields object
      end
    end)
  ...
  defp make_airtable_dataset(record) when is_map(record) do
    formatted_record = case record do
      %{"fields" => _} -> record  # Already in Airtable format
      fields -> %{"fields" => fields}  # Wrap in fields object
    end
