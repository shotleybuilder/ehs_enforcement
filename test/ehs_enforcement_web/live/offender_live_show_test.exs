defmodule EhsEnforcementWeb.OffenderLive.ShowTest do
  ...
  describe "OffenderLive.Show mount" do
    setup do
      ...
      # Earlier case (2023)
      {:ok, earlier_case} = Enforcement.create_case(%{
      ...
      {:ok, warning_notice} = Enforcement.create_notice(%{
    ...
    test "displays enforcement statistics summary", %{conn: conn, offender: offender} do
      ...
      assert html =~ "4 Cases" # total_cases
      assert html =~ "6 Notices" # total_notices
      assert html =~ "£275,000" # total_fines
    ...
    test "shows enforcement timeline with chronological order", %{conn: conn, offender: offender, recent_case: recent_case, major_case: major_case} do
      {:ok, view, html} = live(conn, "/offenders/#{offender.id}")
    ...
    test "displays cases grouped by year", %{conn: conn, offender: offender} do
      ...
      assert html =~ "2024" # Recent year header
      assert html =~ "2023" # Previous year header
      assert html =~ "2022" # Environmental case year
      assert html =~ "2020" # Historical case year
    ...
    test "shows detailed case information in timeline", %{conn: conn, offender: offender, recent_case: recent_case} do
      {:ok, view, html} = live(conn, "/offenders/#{offender.id}")

      # Should show case details
      assert html =~ recent_case.regulator_id
      assert html =~ "£75,000" # recent_case fine
    ...
    test "shows detailed notice information in timeline", %{conn: conn, offender: offender, improvement_notice: improvement_notice} do
      {:ok, view, html} = live(conn, "/offenders/#{offender.id}")
      ...
      assert html =~ "30 days" # compliance period
    end
    ...
    test "identifies repeat offender patterns", %{conn: conn, offender: offender} do
      ...
      assert html =~ "Multiple agencies involved" # HSE and EA
    end
    ...
    test "shows agency breakdown in enforcement history", %{conn: conn, offender: offender} do
      ...
      assert html =~ "Health and Safety Executive" # HSE cases
    ...
    test "displays compliance tracking for notices", %{conn: conn, offender: offender, improvement_notice: improvement_notice} do
      {:ok, view, html} = live(conn, "/offenders/#{offender.id}")
    ...
    test "shows industry context and peer comparison", %{conn: conn, offender: offender} do
      ...
      assert html =~ "Manufacturing" # offender's industry
      
      # Should show comparative metrics
      assert html =~ "Above industry average" # high fine amount
    end
    ...
    test "shows related offenders in same industry/area", %{conn: conn, offender: offender} do
      ...
      {:ok, related_offender} = Enforcement.create_offender(%{
      ...
      assert html =~ "Same Industry" # Manufacturing
      assert html =~ "Same Area" # Manchester
    end

    test "displays risk assessment based on enforcement history", %{conn: conn, offender: offender} do
      {:ok, view, html} = live(conn, "/offenders/#{offender.id}")

      # Should show risk assessment
      assert html =~ "Risk Assessment"
      assert html =~ "High Risk" # Based on multiple cases and large fines
    ...
    test "filters timeline by enforcement type", %{conn: conn, offender: offender} do
      ...
      assert render(view) =~ "HSE-2024-001" # case regulator_id
      refute render(view) =~ "HSE-N-2024-001" # notice regulator_id
    end
    ...
    test "filters timeline by agency", %{conn: conn, offender: offender} do
      ...
      assert render(view) =~ "HSE-2024-001" # HSE case
      refute render(view) =~ "EA-2022-089" # EA case
    end
    ...
    test "filters timeline by date range", %{conn: conn, offender: offender} do
      ...
      assert render(view) =~ "HSE-2024-001" # 2024 case
      refute render(view) =~ "HSE-2020-012" # 2020 case
    end
  ...
  describe "OffenderLive.Show real-time updates" do
    ...
    test "receives real-time updates for new cases", %{conn: conn, offender: offender, hse_agency: hse_agency} do
      {:ok, view, html} = live(conn, "/offenders/#{offender.id}")
    ...
    test "receives real-time updates for new notices", %{conn: conn, offender: offender, hse_agency: hse_agency} do
      {:ok, view, html} = live(conn, "/offenders/#{offender.id}")
  ...
  describe "OffenderLive.Show accessibility" do
    ...
    test "includes proper ARIA labels and semantic structure", %{conn: conn, offender: offender} do
      ...
      assert has_element?(view, "[role='list']") # Timeline as list
      assert has_element?(view, "[role='listitem']") # Timeline items
    end
  ...
  describe "OffenderLive.Show performance" do
    ...
    test "handles large enforcement history efficiently", %{conn: conn, offender: offender} do
      ...
      assert html =~ offender.name
      assert html =~ "50 Cases" # Should show correct total
