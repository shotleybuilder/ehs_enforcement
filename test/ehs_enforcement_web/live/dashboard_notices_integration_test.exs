defmodule EhsEnforcementWeb.DashboardNoticesIntegrationTest do
  ...
  describe "Dashboard Notices Integration" do
    ...
    test "dashboard loads with notices card displaying correct metrics", %{conn: conn, agency: agency, offender: offender} do
      # Create test notices
      ...
      assert html =~ "2"  # Should show 2 total notices
      assert html =~ "Recent (Last 30 Days)"
      assert html =~ "2"  # Should show 2 recent notices
      assert html =~ "Compliance Required"
      assert html =~ "2"  # Both notices require compliance
    ...
    test "browse active notices navigation works", %{conn: conn} do
      ...
      assert result || true  # Test that the click doesn't crash
    end
    ...
    test "notices card updates when new notices are created", %{conn: conn, agency: agency, offender: offender} do
      {:ok, view, html} = live(conn, "/dashboard")
    ...
    test "notices card integrates with dashboard 1x4 grid layout", %{conn: conn} do
      ...
      assert html =~ "ENFORCEMENT CASES"  # First card
      assert html =~ "ENFORCEMENT NOTICES"  # Second card (our notices card)
      assert html =~ "OFFENDER DATABASE"  # Third card
      assert html =~ "REPORTS & ANALYTICS"  # Fourth card
