defmodule EhsEnforcementWeb.DashboardCaseNoticeCountTest do
  ...
  describe "Dashboard Case Count and Notice Count functionality" do
    setup do
      ...
      recent_date1 = Date.add(today, -5)  # 5 days ago
      recent_date2 = Date.add(today, -10) # 10 days ago  
      recent_date3 = Date.add(today, -15) # 15 days ago
      recent_date4 = Date.add(today, -20) # 20 days ago
      recent_date5 = Date.add(today, -25) # 25 days ago
    ...
    test "dashboard displays correct Recent Cases count in stats section", %{conn: conn, expected_case_count: expected_case_count} do
    ...
    test "dashboard displays correct Recent Notices count in stats section", %{conn: conn, expected_notice_count: expected_notice_count} do
      {:ok, view, html} = live(conn, "/dashboard")
    ...
    test "Recent Activity shows all items when 'All Types' filter is active", %{conn: conn, expected_total_count: expected_total_count} do
      {:ok, view, html} = live(conn, "/dashboard")
      ...
      assert html =~ "Court Case", "Should find 'Court Case' in HTML. Found empty state: #{html =~ "No recent enforcement activity"}"
      assert html =~ "Notice"
      ...
      assert row_count == expected_total_count, "Expected #{expected_total_count} activity items but found #{row_count}"
    end

    test "Cases filter shows only court cases with correct count", %{conn: conn, expected_case_count: expected_case_count} do
      ...
      refute html =~ "Improvement Notice", "Should not show Improvement Notice when filtering by cases"
      refute html =~ "Prohibition Notice", "Should not show Prohibition Notice when filtering by cases" 
      ...
      assert row_count == expected_case_count, "Expected #{expected_case_count} case items but found #{row_count}"
    ...
    test "Notices filter shows only notices with correct count", %{conn: conn, expected_notice_count: expected_notice_count} do
      {:ok, view, _html} = live(conn, "/dashboard")
      ...
      assert html =~ "Improvement Notice", "Should show Improvement Notice when filtering by notices"
      assert html =~ "Prohibition Notice", "Should show Prohibition Notice when filtering by notices"
      ...
      assert row_count == expected_notice_count, "Expected #{expected_notice_count} notice items but found #{row_count}"
    ...
    test "struct type detection works correctly for filtering logic", %{cases: cases, notices: notices} do
      ...
      Enum.each(filtered_notices, fn notice_record ->
        ...
        assert notice_record.offence_action_type in ["Improvement Notice", "Prohibition Notice", "Crown Notice"]
        assert Map.get(notice_record, :offence_fine, nil) == nil, "Notices should not have fine amounts"
      end)
    end
    ...
    test "pagination works correctly with filtered counts", %{conn: conn} do
      ...
      if html =~ "Showing" do
        assert html =~ "of 5 results" || html =~ "5 total", "Should show total count in pagination"
      end
      ...
      if html =~ "Showing" do
        assert html =~ "of 2 results" || html =~ "2 total", "Should show case count in pagination when filtering by cases"
      end
      ...
      if html =~ "Showing" do
        assert html =~ "of 3 results" || html =~ "3 total", "Should show notice count in pagination when filtering by notices"
      end
    end
    ...
    test "Recent Activity table structure displays correct data types", %{conn: conn} do
      ...
      assert html =~ "bg-yellow-100 text-yellow-800", "Should have yellow badge styling for notices"
    end
  ...
  describe "Dashboard stats calculations" do
    setup do
      ...
      {:ok, recent_case} = Enforcement.create_case(%{
        ...
        offence_action_date: Date.add(today, -5), # 5 days ago
      ...
      {:ok, recent_notice} = Enforcement.create_notice(%{
        regulator_id: "RECENT-NOTICE-001",
        agency_id: agency.id,
        offender_id: offender.id,
        offence_action_date: Date.add(today, -10), # 10 days ago
