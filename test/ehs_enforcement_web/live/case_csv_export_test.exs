defmodule EhsEnforcementWeb.CaseCSVExportTest do
  ...
  describe "CSV export functionality" do
    setup do
      ...
      {:ok, case1} = Enforcement.create_case(%{
        ...
        offence_breaches: "Manufacturing safety violations, inadequate machine guarding, failure to provide proper training",
      ...
      {:ok, case2} = Enforcement.create_case(%{
        ...
        offence_breaches: "Chemical storage safety breaches with environmental contamination risk",
        last_synced_at: DateTime.utc_now()
      })
    ...
    test "exports filtered cases when filters are applied", %{conn: conn, agencies: [hse_agency, _ea_agency]} do
      {:ok, view, _html} = live(conn, "/cases")
    ...
    test "CSV formats dates consistently", %{conn: conn} do
      ...
      date_pattern = if csv_content =~ "2024-01-15" do
        ~r/\d{4}-\d{2}-\d{2}/
      else
        ~r/\d{2}\/\d{2}\/\d{4}/
      end
      ...
      assert length(dates_found) >= 3 # Should find at least our 3 test dates
    end
    ...
    test "handles empty result set gracefully", %{conn: conn} do
      ...
      lines = String.split(csv_content, "\n")
      assert length(lines) >= 1 # At least header line
    ...
    test "includes export metadata and timestamps", %{conn: conn} do
      ...
      assert csv_content =~ current_date or 
             csv_content =~ "2024" # Should have recent dates
    end
  end
  ...
  describe "CSV export error handling" do
    ...
    test "provides meaningful error messages", %{conn: conn} do
      ...
      if conn.status == 404 do
        assert true # Expected for invalid endpoint
      else
  ...
  describe "CSV export security" do
    test "prevents CSV injection attacks", %{conn: conn} do
      ...
      {:ok, _dangerous_case} = Enforcement.create_case(%{
