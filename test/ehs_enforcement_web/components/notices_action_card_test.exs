defmodule EhsEnforcementWeb.Components.NoticesActionCardTest do
  ...
  describe "notices_action_card/1" do
    ...
    test "calculates and displays correct metrics when notices exist", %{agency: agency, offender: offender} do
      ...
      {:ok, _recent_notice} = Enforcement.create_notice(%{
      ...
      {:ok, _old_notice} = Enforcement.create_notice(%{
      ...
      {:ok, _recent_notice_2} = Enforcement.create_notice(%{
      ...
      assert html =~ "Total Notices"
      assert html =~ "3"  # Total count should be 3
      assert html =~ "Recent (Last 30 Days)"
      assert html =~ "2"  # Recent count should be 2 (within 30 days)
      assert html =~ "Compliance Required"
      assert html =~ "3"  # All have compliance required (nil or future date)
    end
    ...
    test "calculates compliance required correctly for various scenarios", %{agency: agency, offender: offender} do
      today = Date.utc_today()
      past_date = Date.add(today, -10)
      future_date = Date.add(today, 10)

      # Notice with no compliance date (compliance required)
      {:ok, _notice_1} = Enforcement.create_notice(%{
        regulator_id: "TEST001",
        notice_date: today,
      ...
      {:ok, _notice_2} = Enforcement.create_notice(%{
        regulator_id: "TEST002",
        notice_date: today,
        offence_action_date: today,
        compliance_date: future_date,
      ...
      {:ok, _notice_3} = Enforcement.create_notice(%{
        regulator_id: "TEST003",
        notice_date: today,
        offence_action_date: today,
      ...
      assert html =~ "3"
      assert html =~ "Compliance Required"
      assert html =~ "2"  # Only 2 require compliance (nil and future date)
    end
    ...
    test "formats numbers with commas for large values", %{agency: agency, offender: offender} do
      ...
      for i <- 1..1500 do
        {:ok, _notice} = Enforcement.create_notice(%{
      ...
      assert html =~ "1,5"  # Will match "1,500" in the HTML
    end
